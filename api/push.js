// api/push.js — incremental rebuild after 500 isolation test
...
const anonClient = () => createClient(SUPABASE_URL, ANON_KEY, { auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false } });

// Lazy-load web-push at first send so module init doesn't crash when the
// package's CJS/ESM interop behaves oddly under Vercel's runtime.
let _wp = null;
async function getWebPush() {
  if (_wp) return _wp;
  const mod = await import('web-push');
  _wp = mod.default || mod;
  if (pushReady && _wp.setVapidDetails) {
...
export default async function handler(req, res) {
  ...
  // POST/GET /api/push/send-daily — Vercel Cron or manual with x-cron-token
  if ((req.method === 'POST' || req.method === 'GET') && url.includes('send-daily')) {
    ...
    try {
      ...
      console.error('[push/send-daily] error:', err?.message || err);
