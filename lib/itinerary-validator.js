...
/**
 * Normalize European comma-decimal amounts to standard format.
 * "€1.500,50" → "€1,500.50", "€1.500" (ambiguous) → "€1,500"
 */
function normalizeAmount(s) {
  if (!s || typeof s !== 'string') return s;
  // Match currency prefix, then number with dots-as-thousands + comma-decimal
  // Pattern: "€1.500,50" or "1.500,50 EUR"
  return s.replace(/(\d{1,3}(?:\.\d{3})+),(\d{2})\b/g, (_, intPart, dec) => {
    return intPart.replace(/\./g, ',') + '.' + dec;
  }).replace(/(\d{1,3}(?:\.\d{3})+)(?!\.\d)/g, (m) => {
    // "€1.500" (no decimal part) — dots are thousands separators
    if (/^\d{1,3}(\.\d{3})+$/.test(m)) return m.replace(/\./g, ',');
    return m;
  });
}
...
export function validateItinerary(plan) {
  ...
  // ── CHECK 9: Normalize comma-decimal budget amounts ───────────────────────
  const budget = result.rawPlan?.budget;
  if (Array.isArray(budget)) {
    for (const item of budget) {
      if (item.amount) {
        const fixed = normalizeAmount(item.amount);
        if (fixed !== item.amount) {
          fixesApplied.push({ fix: 'comma-decimal', message: `Budget "${item.item || item.label}": ${item.amount} → ${fixed}` });
          item.amount = fixed;
        }
      }
    }
  ...
  // ── CHECK 10: Cross-day duplicate activities ──────────────────────────────
  const activityByLocation = new Map();
  for (const day of days) {
    for (const item of (day.timeline || [])) {
      if (!item.mapQuery || item.type === 'hotel' || item.type === 'transport' || item.type === 'logistics') continue;
      const key = item.mapQuery.toLowerCase().trim();
      if (activityByLocation.has(key)) {
        const prev = activityByLocation.get(key);
        if (prev.day !== day.day) {
          warnings.push({
            day: day.day,
            time: item.time || '',
            check: 'cross-day-duplicate',
            message: `Day ${day.day}: "${item.title}" visits same location as Day ${prev.day} "${prev.title}" (${item.mapQuery})`
          });
        }
      } else {
        activityByLocation.set(key, { day: day.day, title: item.title });
      }
    }
  }
  ...
  for (const day of days) {
    for (const item of (day.timeline || [])) {
      ...
      if (title.includes('airport') || title.includes('check in') || title.includes('check-in') ||
          title.includes('arrive at') || title.includes('taxi') || title.includes('uber')) continue;
      if (!item.from && !item.to) {
        warnings.push({
          day: day.day, time: item.time || '', check: 'transport-missing-route',
          message: `Day ${day.day}: transport "${item.title}" is missing from/to fields — should specify departure and arrival stations/airports`
        });
      }
    }
  }

  // ── CHECK 11: Geographic plausibility (haversine between consecutive days) ─
  const mapStops = result.rawPlan?.mapStops;
  if (Array.isArray(mapStops) && mapStops.length >= 2) {
    const dayCity = new Map();
    for (const day of days) {
      if (day.city) dayCity.set(day.day, day.city.toLowerCase().split(',')[0].trim());
    }
    const stopCoords = new Map();
    for (const s of mapStops) {
      if (s.name && typeof s.lat === 'number' && typeof s.lng === 'number') {
        stopCoords.set(s.name.toLowerCase().split(',')[0].trim(), { lat: s.lat, lng: s.lng });
      }
    }
    ...
    let prevDay = 0;
    for (const day of days) {
      const city = dayCity.get(day.day);
      if (!city || city === prevCity) { prevCity = city; prevDay = day.day; continue; }
      if (prevCity && stopCoords.has(city) && stopCoords.has(prevCity)) {
        const a = stopCoords.get(prevCity);
        const b = stopCoords.get(city);
        const km = haversineKm(a.lat, a.lng, b.lat, b.lng);
        const hasTransport = (day.timeline || []).some(t => t.type === 'transport');
        if (km > 400 && !hasTransport) {
          warnings.push({
            day: day.day, time: '', check: 'geographic-implausible',
            message: `Day ${day.day}: jumps ${Math.round(km)} km from ${prevCity} (Day ${prevDay}) to ${city} with no transport item — likely impossible without a travel day`
          });
        } else if (km > 200) {
          warnings.push({
            day: day.day, time: '', check: 'geographic-long-transit',
            message: `Day ${day.day}: ${Math.round(km)} km transit from ${prevCity} to ${city} — ensure enough travel time is allocated`
          });
        }
      }
      prevCity = city;
