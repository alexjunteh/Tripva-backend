...
const AWIN = {
  ...
  advertisers: {
    ...
    getyourguide: 16148,   // GetYourGuide (via Awin)
    rentalcars:   8340,    // Rentalcars
  },
};

// Direct partner IDs (separate from Awin)
const GYG_PARTNER_ID = process.env.GYG_AID    || '';  // GetYourGuide direct partner ID
const KLOOK_AID      = process.env.KLOOK_AID   || '';  // Klook affiliate ID
...
/**
 * GetYourGuide search URL. Uses direct partner_id if set; otherwise bare URL
 * for wrapping via Awin link builder (see createAwinLink).
 */
export function activityLink({ city, date, activityName = '' }) {
  const params = new URLSearchParams({ q: activityName || city, cmp: 'tripva' });
...
/**
 * Klook search URL — preferred for Asia-Pacific destinations.
 */
export function klookLink({ city, activityName = '' }) {
  const params = new URLSearchParams({ query: activityName || city, cmp: 'tripva' });
...
/** Pick GYG or Klook based on destination — Klook for Asia-Pacific, GYG elsewhere. */
export function activityLinkForDestination({ destination, activityName = '', date = '' }) {
  const dest = (destination || '').toLowerCase();
  const useKlook = KLOOK_AID && KLOOK_REGIONS.some(r => dest.includes(r));
  return useKlook
    ? klookLink({ city: destination, activityName })
    : activityLink({ city: destination, date, activityName });
}
...
export function flightLink({ from, to, date, returnDate = '', travelers = 2 }) {
  ...
  if (affiliateId) params.set('shmarker', affiliateId);
  return `https://www.kiwi.com/us/search/results?${params.toString()}`;
}

export function tripcomFlightLink({ from, to, date, travelers = 2 }) {
  const params = new URLSearchParams({ from, to, date, adult: String(travelers) });
  const code = process.env.TRIPCOM_ALLIANCE_CODE || '';
  if (code) params.set('alliancecode', code);
...
export async function enrichWithAffiliateLinksAsync(state, travelers = 2) {
  ...
  const inferCity = (text) => {
    ...
    if (t.includes('amsterdam')) return 'Amsterdam';
    if (t.includes('berlin')) return 'Berlin';
    if (t.includes('madrid')) return 'Madrid';
    if (t.includes('lisbon') || t.includes('porto')) return 'Lisbon';
    if (t.includes('prague')) return 'Prague';
    if (t.includes('vienna')) return 'Vienna';
    if (t.includes('dubrovnik') || t.includes('split') || t.includes('croatia')) return 'Dubrovnik';
    if (t.includes('santorini') || t.includes('mykonos') || t.includes('athens') || t.includes('greece')) return 'Athens';
    if (t.includes('new york') || t.includes('manhattan') || t.includes('brooklyn')) return 'New York';
    if (t.includes('los angeles') || t.includes('hollywood') || t.includes('venice beach')) return 'Los Angeles';
    if (t.includes('bangkok') || t.includes('chao phraya')) return 'Bangkok';
    if (t.includes('phuket') || t.includes('phi phi') || t.includes('krabi')) return 'Phuket';
    if (t.includes('chiang mai')) return 'Chiang Mai';
    if (t.includes('singapore')) return 'Singapore';
    if (t.includes('kuala lumpur') || t.includes(' kl ') || t.includes('petronas')) return 'Kuala Lumpur';
    if (t.includes('bali') || t.includes('ubud') || t.includes('seminyak') || t.includes('canggu')) return 'Bali';
    if (t.includes('tokyo') || t.includes('shibuya') || t.includes('shinjuku') || t.includes('asakusa')) return 'Tokyo';
    if (t.includes('kyoto') || t.includes('fushimi inari') || t.includes('arashiyama')) return 'Kyoto';
    if (t.includes('osaka') || t.includes('dotonbori')) return 'Osaka';
    if (t.includes('seoul') || t.includes('myeongdong') || t.includes('gangnam')) return 'Seoul';
    if (t.includes('barcelona') || t.includes('sagrada') || t.includes('la rambla')) return 'Barcelona';
    if (t.includes('swiss') || t.includes('zurich') || t.includes('lucerne') || t.includes('interlaken')) return 'Switzerland';
    if (t.includes('maldive')) return 'Maldives';
    if (t.includes('dubai') || t.includes('burj khalifa')) return 'Dubai';
    if (t.includes('istanbul') || t.includes('hagia sophia') || t.includes('bosphorus')) return 'Istanbul';
    if (t.includes('sydney') || t.includes('opera house') || t.includes('bondi')) return 'Sydney';
    if (t.includes('melbourne')) return 'Melbourne';
    if (t.includes('new zealand') || t.includes('queenstown') || t.includes('auckland')) return 'New Zealand';
    return '';
  };

  // Extract city from mapQuery like "Anne Frank House, Amsterdam" → "Amsterdam"
  const extractCityFromMapQuery = (mapQuery) => {
    if (!mapQuery) return '';
    const parts = String(mapQuery).split(',');
  ...
  // Timeline activities — stamp each activity item with a bookUrl
  if (plan.days && plan.days.length > 0) {
    const tripDestination = plan.trip?.destination || '';
    plan.days = plan.days.map((day) => {
      if (!day.timeline) return day;
      const timeline = day.timeline.map((tl) => {
        if (tl.type !== 'activity' || tl.bookUrl) return tl;
        const cityHint = extractCityFromMapQuery(tl.mapQuery)
          || inferCity(day.title + ' ' + tripDestination)
          || tripDestination;
        const bookUrl = activityLinkForDestination({ destination: cityHint || tripDestination, activityName: tl.title });
        return { ...tl, bookUrl };
      });
      return { ...day, timeline };
    });
