...
// 12Go specializes in SEA overland transport; Japan/Korea/Taiwan have their own
// rail networks better served by Rail Ninja.
const GO12_REGIONS = ['thailand','bali','indonesia','singapore','malaysia','vietnam','cambodia','hong kong','philippines','sri lanka','nepal','india','maldives','myanmar'];
...
function enrichPlan(state, travelers = 2) {
  ...
  if (plan.tickets && plan.tickets.length > 0) {
    ...
    const useSEA = GO12_REGIONS.some(r => tripDest.includes(r));
    plan.tickets = plan.tickets.map(t => {
      if (t.bookUrl && /marker=|aff_id=/.test(t.bookUrl)) return t;
      const linkFn = useSEA ? go12Url : railNinjaUrl;
  ...
  if (plan.days && plan.days.length > 0) {
    ...
    const useSEA = GO12_REGIONS.some(r => destLower.includes(r));
      ...
      return {
        ...
        timeline: day.timeline.map(tl => {
          ...
          if (tl.type === 'transport') {
            ...
            let to = tl.to || '';
            if (!from || !to) {
              const mq = tl.mapQuery || '';
              const mqRoute = mq.match(/^(.+?)\s+to\s+(.+?)(?:,|$)/i);
              if (mqRoute) {
                from = from || mqRoute[1].trim();
                to = to || mqRoute[2].trim();
              }
              if (!to) {
                const titleTo = (tl.title || '').match(/\bto\s+([A-Z][A-Za-z\s]+)/);
                if (titleTo) to = titleTo[1].trim();
              }
              if (!from && dayIdx > 0) {
                const prevDay = plan.days[dayIdx - 1];
                from = prevDay?.heroSeed || '';
                if (from) from = from.charAt(0).toUpperCase() + from.slice(1);
              }
              if (!to) {
                to = day.heroSeed || '';
                if (to) to = to.charAt(0).toUpperCase() + to.slice(1);
              }
            }
            return { ...tl, bookUrl: linkFn({ from, to, date: day.date || '' }) };
          }
          if (tl.type === 'activity') {
