...
function isAllowedOrigin(origin) {
  ...
  // Allow Cloudflare Pages branch previews
...
const COSTLY_RATE_WINDOW_MS = 60_000;
const costlyRateLimitStore = new Map();

export function checkRateLimitCostly(ip) {
  const now = Date.now();
  let entry = costlyRateLimitStore.get(ip);
  if (!entry || now > entry.resetAt) {
    entry = { count: 0, resetAt: now + COSTLY_RATE_WINDOW_MS };
  }
  entry.count++;
  costlyRateLimitStore.set(ip, entry);
  if (costlyRateLimitStore.size > 5000) {
    for (const [k, v] of costlyRateLimitStore) {
      if (now > v.resetAt) costlyRateLimitStore.delete(k);
    }
  }
  return {
    allowed: entry.count <= COSTLY_RATE_LIMIT,
    remaining: Math.max(0, COSTLY_RATE_LIMIT - entry.count),
    resetAt: new Date(entry.resetAt).toISOString(),
  };
}
