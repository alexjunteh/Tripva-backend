...
export const CITY_PHOTOSPOTS = {
  "venice": [
    ...
    },
    {
      ...
      bestTime: "10:30–12:00, sunny morning",
...
export function enrichPlan(plan) {
  ...
  for (const day of plan.days) {
    if (day.imageUrl && day.imageUrl.includes('loremflickr')) day.imageUrl = '';
    ...
    if (Array.isArray(day.timeline)) {
      ...
      if (spots.length > 0) {
        // De-dupe by id OR by name/title keywords
        ...
        const existingKeywords = new Set(
          day.timeline
            .filter(t => t.type === 'photospot')
            .flatMap(t => (t.title || t.name || '').toLowerCase().split(/[\s,–—]+/).filter(w => w.length > 3))
        );

        // Also skip if a matching non-photospot activity covers the same place
        const activityKeywords = new Set(
          day.timeline
            .filter(t => t.type !== 'photospot')
            .flatMap(t => (t.title || '').toLowerCase().split(/[\s,–—]+/).filter(w => w.length > 4))
        );

        for (const spot of spots) {
          if (existingIds.has(spot.id)) continue;

          // Check keyword overlap with existing photospots
          const spotWords = (spot.name || '').toLowerCase().split(/[\s,–—]+/).filter(w => w.length > 3);
          const nameOverlap = spotWords.some(w => existingKeywords.has(w));
          if (nameOverlap) continue;

          // Extract best time for time slot (e.g. "06:30–07:30" → "06:30")
          const timeMatch = (spot.bestTime || '').match(/\b(\d{2}:\d{2})/);
          const assignedTime = timeMatch ? timeMatch[1] : null;

          // Skip if time slot already occupied by another photospot
          if (assignedTime) {
            const slotTaken = day.timeline.some(t => t.time === assignedTime);
            if (slotTaken) continue;
          }

          // Map schema: places.js uses name/description, dashboard needs title/detail
          day.timeline.push({
            id:       spot.id,
            type:     'photospot',
            time:     assignedTime || '17:00',
            title:    spot.name,
            detail:   spot.description || spot.detail || '',
            tip:      spot.tip || '',
            lat:      spot.lat,
            lng:      spot.lng,
            mapUrl:   spot.mapUrl,
            mapQuery: spot.mapUrl ? spot.mapUrl.replace('https://maps.google.com/?q=', '') : undefined,
