...
- Gear already owned: ${gear}. For gear NOT listed, note rental shops with daily rates; for gear listed, skip rental notes. Be explicit about what's missing that the traveler MUST buy or rent.
...
- Weather-sensitive planning: note trail conditions, best seasons, rainy-day fallbacks for each activity.
- Include permits/advance-booking warnings for popular trails (note which need reserving and when).
- Food: hearty, high-calorie options for activity days. Note where to refuel mid-trail.
- Transport: emphasize 4WD rentals, shuttle services to trailheads, public transit from town to trail where relevant.
