...
RULE 2 — MINIMUM 7 TIMELINE ITEMS PER DAY (MANDATORY — HARD FLOOR):
  Every day MUST have AT LEAST 7 timeline items. Aim for 8-10.
  This is a HARD MINIMUM — no exceptions, not even ARRIVAL or DEPARTURE days.
  Count before finalizing each day: if < 7, add meals, nearby walks, or logistics.
  Each item MUST have a specific time in HH:MM 24h format (e.g. "09:00").
  A "full day" with vague descriptions is NOT acceptable.
  SELF-CHECK: After generating ALL days, re-count each day's timeline. If ANY
  day has fewer than 7 items, add items until it reaches 7.

RULE 3 — TIMELINE ITEM REQUIRED FIELDS (MANDATORY):
  Every timeline item MUST include ALL of these fields:
    ...
    - "type": one of "transport" | "activity" | "meal" | "hotel" | "food" | "logistics" | "photospot"
    - "mapQuery": searchable Google Maps string for this location
  Photospot items additionally MUST include: tip, lat, lng (see RULE 11).
...
RULE 4 — BUDGET IS THE USER'S ENVELOPE — ALLOCATE TO IT (MANDATORY):
  ...
  ALWAYS include a "Return flights" item in category:"transport" unless the trip is explicitly a local road trip — flights are ALWAYS the biggest budget item for international/domestic air travel.
  ALWAYS include at least one "Activities" or "Attractions" item (museums, tours, entry fees) in category:"activities".
...
RULE 5 — HOTEL REQUIRED FIELDS (MANDATORY):
  Every item in hotels[] MUST include:
    ...
    - "address": full street address of the hotel (used for emergency contact info)
...
RULE 9 — DAY CITY FIELD (MANDATORY):
  Every day object MUST include a "city" field: the primary city for that day (e.g. "Rome", "Bali, Indonesia", "Koh Samui, Thailand").
  Use "City, Country" format for any city whose name is ambiguous or shared with other places (e.g. Bali, Nice, Florence, Victoria, Santiago).
  For globally unambiguous cities (Paris, Tokyo, London, Sydney) the city name alone is fine.
...
RULE 10 — LOCAL TIPS PER CITY (MANDATORY):
  ...
  Also generate a top-level "tips" array with one entry per city:
    { "category": "<City> Tips", "icon": "<city emoji>", "items": ["tip 1", "tip 2", "tip 3"] }
  This populates the dedicated Tips tab in the app.
...
RULE 13 — STARS AND REVIEWS ON ACTIVITIES (MANDATORY):
  Every meal, food, and activity item MUST include:
...
RULE 14 — HEROSEQUENCE & IMAGE (MANDATORY):
  ...
  Use city/neighbourhood names (e.g. "shinjuku", "asakusa", "shibuya", "fuji", "kawagoe", "amsterdam", "rome").
  NEVER use compound strings like "tokyo-arrival" or "amsterdam-canals" — use ONLY a single keyword with no hyphens.

  Also include "imageUrl" — Set "imageUrl" to "" (empty string) — the frontend will use its own curated Unsplash images based on the heroSeed.

  The trip-level "trip" object MUST include an "emoji" field: a single emoji representing the trip's dominant destination or theme (e.g. "🗼" for Paris, "🌺" for Bali, "🏯" for Japan).

════════════════════════════════════════════════════════════════
RULE 15 — GEOGRAPHIC ROUTING PROTOCOL (MANDATORY):
════════════════════════════════════════════════════════════════

STEP 1 — PLOT BEFORE YOU PLAN:
  Before writing ANY day, mentally plot every destination on a map.
  Order them as a single sweep (line or loop) — whichever covers the
  least total distance. NEVER zigzag or revisit a region after leaving it.
  Exception: returning to the gateway/airport city for the departure
  flight is expected and NOT backtracking (e.g. Tokyo→Kyoto→Hiroshima→Tokyo
  for a Singapore-Tokyo flight is a valid loop).

STEP 2 — CLASSIFY EACH DAY:
  Every day is one of:
    • EXPLORE DAY — stay in the same city as yesterday. Full 8-10 activities.
    • TRAVEL DAY — move between cities. Include the transit as a timeline
      item (type: "transport") with realistic departure+arrival times.
      Plan 4-6 destination activities (you lose part of the day to travel),
      but the TOTAL timeline items (transport + hotel + activities + meals)
      must still reach the RULE 2 minimum of 7.
    • ARRIVAL/DEPARTURE DAY — lighter activity load near airport/station,
      but total timeline items must still reach 7 (include arrival transport,
      hotel check-in/out, meals, and nearby activities).

STEP 3 — DISTANCE REALITY CHECK (HARD CONSTRAINT):
  All activities within a single day MUST be in the SAME city or within
  a 100 km radius. If two locations are >100 km apart, they CANNOT be
  on the same day unless the day is classified as a TRAVEL DAY with an
  explicit transport timeline item between them.

  COMMON MISTAKES TO AVOID:
  • New Zealand: Milford Sound is 300 km from Queenstown (4h drive ONE WAY),
    1,200 km from Wellington. It is NEVER a day trip from Wellington.
  • Japan: Hiroshima is 800 km from Tokyo — that is a travel day, not a
    side trip.
  • Europe: Barcelona to Madrid is 620 km — require a flight or AVE train
    (2.5h), and count it as a travel day.
  • Always check: "Can a real human drive/train between these two points
    AND do activities AND return in one day?" If no → separate days.

STEP 3.5 — SMART TRANSPORT METHOD SELECTION (MANDATORY FOR TRAVEL DAYS):
  Every TRAVEL DAY must include a transport timeline item with the BEST
  mode for that specific leg. Choose based on distance + region + convenience:

  DECISION MATRIX — pick the FIRST match:
  ┌─────────────────┬───────────────────────────────────────────────┐
  │ Condition        │ Mode                                          │
  ├─────────────────┼───────────────────────────────────────────────┤
  │ Island → island  │ Ferry or flight (ferry if <4h, flight if >4h) │
  │ <150 km          │ Train, bus, or rental car (regional default)  │
  │ 150-400 km       │ High-speed rail if available, else bus/drive  │
  │ 400-800 km       │ Flight OR fast train (choose faster option)   │
  │ >800 km          │ Flight (always)                               │
  │ No rail/road     │ Ferry or domestic flight                      │
  └─────────────────┴───────────────────────────────────────────────┘

  REGIONAL TRANSPORT INTELLIGENCE — use the local best option:
  • Japan: Shinkansen (bullet train) for 150-900 km. Tokyo→Kyoto = 2h15m
    Nozomi, Tokyo→Hiroshima = 4h. JR Pass covers unlimited rides.
  • Europe: High-speed rail (TGV, ICE, AVE, Eurostar, Thalys, Frecciarossa).
    Paris→Amsterdam = 3h Thalys. Rome→Florence = 1h30m Frecciarossa.
    Budget airlines (Ryanair, EasyJet) for >800 km or cross-region.
  • Southeast Asia: Budget flights (AirAsia, VietJet, Scoot) between cities.
    Overnight buses/trains for 200-500 km where scenic (Vietnam, Thailand).
  • New Zealand: Rental car is default — limited rail, no high-speed trains.
    Inter-island = Interislander ferry (Wellington↔Picton, 3.5h) or flight.
  • Greek Islands: Ferry (Blue Star, SeaJets) between islands. Flight only
    for Athens↔distant islands (Crete, Rhodes >8h ferry).
  • UK: Train (LNER, GWR, Avanti). London→Edinburgh = 4h20m LNER.
  • Australia: Flight for intercity (Sydney→Melbourne 1.5h flight vs 9h drive).
  • South America: Flight between countries/distant cities. Bus for 4-8h scenic.

  TRANSPORT EMOJI RULES (MANDATORY — use the CORRECT emoji for the mode):
    🚄 = Train / Rail / Shinkansen / HSR / Metro
    ✈️ = Flight / Airplane
    ⛴️ = Ferry / Boat / Ship / Cruise
    🚗 = Car / Drive / Rental car / Taxi
    🚌 = Bus / Coach / Shuttle
    🛺 = Tuk-tuk / Rickshaw
    🚇 = Subway / Underground / MRT
    🚠 = Cable car / Gondola / Funicular
  NEVER use a train emoji (🚄) for a ferry — use ⛴️. Match the emoji to the
  actual transport mode, not the previous item's emoji.

  TRANSPORT TIMELINE ITEM REQUIREMENTS:
  Every transport item between cities MUST include:
    - "type": "transport"
    - "title": "[Correct mode emoji] [Mode] to [Destination]" — e.g. "🚄 Shinkansen to Kyoto",
      "✈️ Flight to Bali", "⛴️ Ferry to Santorini", "🚗 Drive to Queenstown"
    - "detail": Include operator name, route, realistic duration, class/tier,
      and one practical tip (e.g. "Book JR Pass before arriving in Japan",
      "Check in online 24h before for best prices")
    - "from": departure city/station/airport
    - "to": arrival city/station/airport
    - "time": realistic departure time (morning for long trips, flexible for short)
    - "mapQuery": "[Station/Airport/Port Name], [City], [Country]"

  EXAMPLES OF GOOD TRANSPORT ITEMS:
  ✅ {"time":"08:30","title":"🚄 Shinkansen Nozomi to Kyoto","detail":"JR Central Nozomi #225, Tokyo Station → Kyoto Station, 2h15m. Reserved seat in car 7-10. Buy ekiben (train bento) at the station for the ride. JR Pass holders: reserve seats free at any JR ticket office.","type":"transport","from":"Tokyo Station","to":"Kyoto Station","mapQuery":"Tokyo Station, Tokyo, Japan"}
  ✅ {"time":"07:00","title":"⛴️ Interislander Ferry to Picton","detail":"Interislander ferry, Wellington → Picton, 3.5h crossing through the Marlborough Sounds. Book the Kaitaki vessel for vehicle deck + scenic upper deck. Arrive 60min before sailing.","type":"transport","from":"Wellington Ferry Terminal","to":"Picton Ferry Terminal","mapQuery":"Interislander Ferry Terminal, Wellington, New Zealand"}
  ✅ {"time":"10:00","title":"✈️ Flight to Bali","detail":"AirAsia AK 375, Bangkok Don Mueang → Bali Ngurah Rai, 4h30m. Book 2-3 weeks ahead for best fares (~$60-80). Carry-on only saves $15.","type":"transport","from":"Bangkok Don Mueang Airport","to":"Bali Ngurah Rai Airport","mapQuery":"Don Mueang International Airport, Bangkok, Thailand"}
  ❌ {"time":"09:00","title":"Travel to next destination","type":"transport"} — NO! Missing mode, duration, from/to, operator, detail.

STEP 4 — mapQuery MUST MATCH THE DAY'S CITY:
  Every timeline item's mapQuery MUST contain the correct city name.
  NEVER write "Milford Sound Lodge, Auckland" when the lodge is near
  Milford Sound — write "Milford Sound Lodge, Milford Sound, New Zealand".

STEP 5 — NO DUPLICATE ACTIVITIES ACROSS DAYS:
  Never schedule the same attraction, restaurant, or venue on multiple days.
  If a traveler stays 3 days in Tokyo, each day visits DIFFERENT neighborhoods
  and attractions. Repeating Sky Tower on Day 1, Day 2, and Day 8 is a failure.

════════════════════════════════════════════════════════════════
ADDITIONAL ROUTING & PLANNING RULES:
════════════════════════════════════════════════════════════════
- Route-first: design geography before hotels. No backtracking.
- Hotel within 10min walk of main station/transport hub when possible.
- Transport mode follows STEP 3.5 decision matrix — NOT always train.
  Use rail when it's fastest, fly when rail is >4h, drive/ferry when
  region demands it. Smart ≠ train-first; smart = fastest+convenient.
- Max 2.5h daily transport on EXPLORE days (short transfers within city).
  TRAVEL days can have longer transit — that's the whole point.
- 2-4 nights minimum per city (unless it's a transit stop).
- No Gulf airport routing (Dubai, Doha, Abu Dhabi) as transit hubs
  unless the Gulf city is an actual destination on the trip.
- Hotels must have real checkin/checkout dates.
- Budget items must have realistic price estimates.
- Budget MUST include transport costs between cities — flights, train
  tickets, ferry fares, car rental. These are often 20-30% of total budget.
- occasion field accuracy: "weekend" = 1-3 days, "short break" = 4-5 days,
  "vacation" = 6-14 days, "extended" = 15+ days. Match the trip length.
...
Example of ONE correctly structured day (your output must match this pattern for every day):
{
  ...
  "timeline": [
    {
      "time": "10:30",
      "title": "🚄 Thalys from Brussels to Amsterdam",
      "detail": "Thalys high-speed train, Brussels-Midi → Amsterdam Centraal, 1h50m. Reserved seat in comfort class. Follow signs to the GVB tram stop just outside the main entrance for transport into the city centre.",
      "type": "transport",
  ...
  "rawPlan": {
    ...
    "days": [{
      ...
      "timeline": [
        ...
        { "time": "16:00", "title": "Trastevere Walk", "detail": "Explore the cobbled streets", "type": "activity", "mapQuery": "Trastevere, Rome", "stars": 4.3, "reviews": "1,890 reviews" },
    ...
    "budget": [
      { "item": "Flights", "amount": "RM 2,400", "note": "Return KUL-FCO via MH", "status": "pending", "category": "transport", "pct": 35 },
      { "item": "Hotels (12 nights)", "amount": "RM 6,000", "note": "Avg RM 500/night", "status": "pending", "category": "accommodation", "pct": 42 }
    ],
    "tips": [
      {
        "category": "Rome Tips",
        "icon": "🏛️",
        "items": ["Book Colosseum tickets online to skip 2h queues", "Validate transit tickets before boarding or face €50 fines", "Most museums close Monday — plan accordingly"]
      }
    ],
    "urgent": [
...
export function deriveConstraintLines(anchors) {
  ...
  for (const a of anchors) {
    ...
    // Arrival anchor: block activities until arrival + transit buffer
    if (a.arrivalTime && a.date) {
      const [h, m] = a.arrivalTime.split(':').map(Number);
      const bufMin = h * 60 + m + 90;
      if (Math.floor(bufMin / 60) >= 24) {
        lines.push(
          `${a.date}: LATE ARRIVAL DAY — ${a.type} arrives ${a.from || '?'}→${a.to || '?'} at ${a.arrivalTime}. ` +
          `Buffer extends past midnight; no sightseeing activities practical on this day.`
        );
      } else {
        ...
        lines.push(
          ...
          `(${a.type} arrives ${a.from || '?'}→${a.to || '?'} at ${a.arrivalTime}, +90 min transit). ` +
          `First activity no earlier than ${bufH}:${bufM}.`
        );
      }
    }

    // Departure anchor — emitted regardless of whether arrivalTime is also set
    // (transit days have both; both constraints must be shown to the planner)
    if (a.departureTime && a.date) {
      const [h, m] = a.departureTime.split(':').map(Number);
      const bufMin = h * 60 + m - 120; // 2h airport buffer
      const safeBufMin = Math.max(0, bufMin);
      const bufH = String(Math.floor(safeBufMin / 60)).padStart(2, '0');
      const bufM = String(safeBufMin % 60).padStart(2, '0');
      lines.push(
...
/**
 * Build the FIXED BOOKING ANCHORS block shared by both plan and skeleton prompts.
 * @param {Array|null} anchors
 * @returns {string}
 */
function buildAnchorsBlock(anchors) {
  if (!anchors || anchors.length === 0) return '';
  return 'FIXED BOOKING ANCHORS — HARD CONSTRAINTS (do not change these dates or cities):\n' +
    'The traveler has confirmed bookings. Plan around them exactly:\n' +
    anchors.map(a => {
      if (a.type === 'flight' || a.type === 'train') {
        return (a.type === 'flight' ? 'FLIGHT' : 'TRAIN') + ': ' +
          (a.from || '?') + ' → ' + (a.to || '?') +
          (a.date ? ' on ' + a.date : '') +
          (a.departureTime ? ' depart ' + a.departureTime : '') +
          (a.arrivalTime ? ' arrive ' + a.arrivalTime : '') +
          (a.flightNumber ? ' [' + a.flightNumber + ']' : '') +
          (a.confirmationRef ? ' ref:' + a.confirmationRef : '');
      }
      if (a.type === 'hotel') {
        return 'HOTEL: ' + (a.hotelName || 'pre-booked hotel') + ' in ' + (a.city || 'unknown') +
          (a.checkinDate ? ' check-in ' + a.checkinDate : '') +
          (a.checkoutDate ? ' check-out ' + a.checkoutDate : '') +
          (a.confirmationRef ? ' ref:' + a.confirmationRef : '');
      }
      return a.summary || JSON.stringify(a);
    }).join('\n') + '\n' +
    ...
    (() => {
      ...
      return constraintLines ? 'PER-DAY TIME CONSTRAINTS (hard — do not violate):\n' + constraintLines + '\n' : '';
    })();
}
...
export function buildPlanPrompt(input) {
  const { destination, startDate, endDate, travelers, budget, style, interests, homeCity, transportModes } = input;
  ...
  const currencyHint =
    budget.includes('RM') || budget.includes('MYR') ? 'MYR (show amounts in RM)' :
    /SGD|S\$/.test(budget) ? 'SGD (show amounts in S$)' :
    /AUD|A\$/.test(budget) ? 'AUD (show amounts in A$)' :
    /HKD|HK\$/.test(budget) ? 'HKD (show amounts in HK$)' :
    /TWD|NT\$/.test(budget) ? 'TWD (show amounts in NT$)' :
    /JPY|¥/.test(budget) ? 'JPY (show amounts in ¥)' :
    /KRW|₩/.test(budget) ? 'KRW (show amounts in ₩)' :
    /THB|฿/.test(budget) ? 'THB (show amounts in ฿)' :
    /IDR/.test(budget) ? 'IDR (show amounts in Rp)' :
    /INR|₹/.test(budget) ? 'INR (show amounts in ₹)' :
    /PHP/.test(budget) ? 'PHP (show amounts in ₱)' :
    budget.startsWith('€') ? 'EUR' :
    budget.startsWith('£') ? 'GBP' :
    /^\$|USD/.test(budget) ? 'USD' : 'local currency';

  const langInstruction = getLangInstruction(input);
  const archetypeBlock = getArchetypeInstruction(input);
  const destList = Array.isArray(input.destinations) && input.destinations.length > 1 ? input.destinations : null;
  const destBlock = destList
    ? 'MULTI-DESTINATION ROUTE — REQUIRED STOP SEQUENCE:\n' +
      'The traveler wants to visit these destinations in order. Route them efficiently (no backtracking):\n' +
      destList.map((d, i) => '  ' + (i + 1) + '. ' + d).join('\n') + '\n' +
      'Give each destination at least 2 nights. Use "' + input.destination + '" as the trip name.\n'
    : '';

  const anchors = Array.isArray(input.bookingAnchors) && input.bookingAnchors.length > 0
    ? input.bookingAnchors : null;
  const anchorsBlock = buildAnchorsBlock(anchors);

  const transportLine = Array.isArray(transportModes) && transportModes.length > 0
    ? '\n- Preferred transport: ' + transportModes.join(', ') + ' (plan inter-city routes using these modes where feasible)'
    : '';
...
- Total budget: ${budget}
- Travel style: ${style}
- Interests: ${interests.join(', ')}${transportLine}
...
- tickets[]: ALWAYS leave as an empty array [] — never populate this field
- urgent[]: include any must-book-ahead items (trains, attractions, tours) with real URLs
...
export function buildSkeletonPrompt(input) {
  ...
  const selectedSpots = Array.isArray(input.selectedSpots) ? input.selectedSpots : [];
  const spotsBlock = selectedSpots.length > 0
    ? `REQUIRED STOPS — USER SELECTED THESE HIGHLIGHTS:\nThe traveler has specifically chosen these places. Build the itinerary to include ALL of them. Distribute them naturally across the days (do not clump them all on one day):\n${selectedSpots.map(s => `- ${s.name}${s.destination ? ` — in ${s.destination}` : ''} (${s.category}): ${s.description}`).join('\n')}\n`
    : '';
...
day, city, date, title, subtitle, emoji, heroSeed, imageUrl, highlights, localTips, photos
DO NOT include a \`timeline\` array for any day. The timeline for each day will be filled in separately.
"city" = primary city name for that day. Use "City, Country" for ambiguous names (e.g. "Bali, Indonesia", "Nice, France"). Required for hotel matching.
"date" = YYYY-MM-DD date for that day (startDate + day-1).

IMPORTANT — hotels[] and budget[] and tips[] must NOT be empty:
- hotels[]: Include AT LEAST one hotel per city. Every city that has an overnight stay must have a hotel entry. Hotels MUST include checkin/checkout dates AND address.
- budget[]: Include AT LEAST 10 budget items covering: return flights (ALWAYS include unless road trip), accommodation per city, meals, activities, local transport, and misc. Every item needs a "pct" field. NEVER return empty arrays.
- tips[]: Include one entry per city: { "category": "<City> Tips", "icon": "<emoji>", "items": ["3-5 practical tips"] }. This populates the Tips tab.
- mapStops[]: Include one stop per city with real GPS coordinates.
- urgent[]: Include any must-book-ahead items.
...
export function buildDayPrompt(skeleton, dayNum, input) {
  ...
  const city = day.city || dayTitle;

  // Build list of other cities on this trip for negative constraint
  const allDays = skeleton?.rawPlan?.days ?? [];
  const otherCities = [...new Set(allDays.map(d => d.city).filter(c => c && c !== city))];
  const otherCitiesWarning = otherCities.length
    ? `\nDO NOT use any attractions from: ${otherCities.join(', ')}. Those are different days.`
    : '';

  return `YOU ARE GENERATING A DAY IN: ${city.toUpperCase()}, ${input.destination.toUpperCase()}

DAY ${dayNum} of "${tripName}": ${dayTitle}
${daySubtitle ? `Subtitle: ${daySubtitle}` : ''}
${day.highlights?.length ? `MUST-INCLUDE these ${city} highlights: ${day.highlights.join(', ')}` : ''}
${relevantHotel ? `Hotel: ${relevantHotel.name}, ${relevantHotel.city}` : ''}

TRAVELER INFO:
- Country/region: ${input.destination}
- City for THIS day: ${city}
- Travel style: ${input.style}
- Interests: ${input.interests.join(', ')}
- Party size: ${input.travelers}

${archetypeBlock ? archetypeBlock + '\n\n' : ''}Generate the complete hour-by-hour timeline for this day in ${city}, from wake-up to sleep.
Include transport, meals, activities, and check-in/check-out as relevant.
Every mapQuery MUST include "${city}" as the city component.

CRITICAL QUALITY RULES:
1. REAL PLACES ONLY — use actual venue names that exist in ${city}. NEVER use
   generic placeholders like "Day 1 Garden" or "Local Restaurant". Use the
   real name: "Shinjuku Gyoen" not "Tokyo Garden", "Ichiran Ramen Shibuya"
   not "Noodle Shop".
2. CITY ACCURACY — every venue MUST physically be in ${city}.${otherCitiesWarning}
3. USE THE HIGHLIGHTS — the highlights listed above are ${city}'s top attractions.
   Build the day around them.

HARD RULES FOR THIS DAY TIMELINE (MANDATORY — NO EXCEPTIONS):
- ONLY generate items physically located in ${city}.
